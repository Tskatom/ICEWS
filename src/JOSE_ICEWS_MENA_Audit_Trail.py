...
def extend_surr(surr, warn):
    ...
    #icews_daily = surr["icews_features"]
    icews_daily = ""
    icews_surr_values = surr['values']
    icews_columns = surr['columns']
    day = surr['day']
    icews_daily = {day: {icews_columns[i]:icews_surr_values[i] for i in range(len(icews_columns))}}
    ...
    start_day = (parser.parse(event_date) - timedelta(days=30)).strftime("%Y-%m-%d")
