...
def group_cities(line):
    country = line[-3]
    city = line[-6]
    if country == 'Brazil':
        return "%s-%s" %(country,line[-4])
    if country == 'Egypt' and city == 'Tahrir Square':
        return "%s-%s" %(country, 'Cairo')
    return "%s-%s" %(country,city)

# group data by city
data_co = group_by(lines[1:], key=group_cities)
...
osi_countries = set(["argentina-buenos aires", "bahrain-manama", "brazil-distrito federal", "chile-santiago", 
                     "colombia-bogota", "ecuador-quito", "el salvador-san salvador", "egypt-cairo", "iraq-baghdad",
                     "jordan-amman", "libya-tripoli", "mexico-mexico city", "paraguay-asuncion", 
                     "saudi arabia-riyadh","syria-damascus", "uruguay-montevideo", "venezuela-caracas"])
