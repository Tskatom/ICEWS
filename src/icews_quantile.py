...
class Mena():
    ...
    def quantile_reg(self, event_type, event_date, end_date, quantile, feature_folder):
        location = self.location
        country = self.country
        # load the data information
        latest_version = max(os.listdir(feature_folder))
        count_folder = os.path.join(feature_folder, "%s/Count" % (latest_version,))
        if self.pred_level == "city":
            count_file = os.path.join(count_folder, "%s-%s" % (country.replace(" ", "_"),
                                                               location.replace(" ", "_")))
        elif self.pred_level == "country":
            count_file = os.path.join(count_folder, "%s" % country.replace(" ", "_"))
        count_df = pds.DataFrame.from_csv(count_file, sep='\t')
        del count_df['20']
        start_day = count_df.index[0]
        date_range = pds.date_range(start_day, end_date)
        count_df = count_df.reindex(date_range).fillna(0)
        count_df = count_df.resample('W', how='sum').fillna(0)
        column_names = list(count_df.columns.values)
        features = [col for col in column_names if col != event_type[2:]]
        X = np.log(count_df[features].values + 1)
        Y = np.log(count_df[[event_type[2:]]].values + 1)

        # we include previous two weeks Y as features toa
        X = np.hstack([np.ones((len(Y[1:,]), 1)), X[1:,], Y[1:,], Y[:-1,]])
        trainX = X[:-1,]
        trainY = Y[2:,]

        testX = X[-1]
        model = QuantReg(trainY, trainX)
        fitted = model.fit(q=quantile)
        pred = fitted.predict(testX)
        predY = max(np.floor(np.exp(pred)) - 1, 0)
        surrogate = wrap_quantile_surr(count_df)
        #start to wrap the warning
        if self.pred_level == "country":
            event_location = COUNTRY_LOCATION[country]
        elif self.pred_level == "city":
            event_location = CITY_LOCATION[country]
        else:
            print "Please Enter Correct prediction Level [%s]" % pred_level
            sys.exit()
        model_desc = "Quantile Regressive Model"
        comment = "ICEWS %s Prediction use %s " % (event_type, model_desc)

        warning = wrap_warning(surrogate, event_type, predY,
...
def wrap_warning(surrogate, event_type, prediction, location, event_date,
    ...
    if domain is None:
        domain = conn.create_domain('warnings')
...
def wrap_quantile_surr(icews_dataframe):
    surrogate = {}
    surrogate['date'] = datetime.utcnow().isoformat()
    surrogate["icews_series"] = {}
    columns = list(icews_dataframe.columns.values)
    for col in columns:
        surrogate["icews_series"][col] = series2dict(icews_dataframe[col])
    surrogate = message.add_embers_ids(surrogate)
    return surrogate
...
def predict_mena(args):
    ...
    warn_file = os.path.join(out_folder, "WARN_MENA_ICEWS_%s" % currtime)
    surr_file = os.path.join(out_folder, "SURR_MENA_ICEWS_%s" % currtime)
    ...
    for country in MENA_COUNTRY:
        for event_type in event_types:
            ...
            try:
                ...
                end_date = (datetime.now() - timedelta(days=week_day+1)).strftime("%Y-%m-%d")
                country_warn, country_surr = country_mena.quantile_reg(event_type,
                                                                       event_date,
                                                                       end_date,
                                                                       quantile,
                                                                       feature_folder)
                city_warn, city_surr = city_mena.quantile_reg(event_type,
                                                              event_date,
                                                              end_date,
                                                              quantile,
                                                              feature_folder)
...
def parse_args():
    ...
    ap.add_argument('--feature_folder', type=str, help='feature_folder',
                    default='../data/features/icews/')
